...
function useDragOnPlane(y = 0) {
  ...
  const intersect = (event: any) => {
    ...
    raycaster.setFromCamera(new THREE.Vector2(x, yN), camera);
...
const MODEL_MAP: Partial<
  Record<
    PlacedItemType["kind"],
    { url: string; rotationOffsetY?: number; scale?: number }
  >
> = {
  chair: {
    url: "/assets/models/glb/PED-SP-BF.glb",
    rotationOffsetY: Math.PI / 2,
    scale: 1,
  },
  table: {
    url: "/assets/models/glb/DESK-BF.glb",
    rotationOffsetY: Math.PI / 2,
    scale: 1,
  },
  door: {
    url: "/assets/models/glb/door.glb",
    rotationOffsetY: Math.PI / 2,
    scale: 1,
  }, // Fixed door rotation
  ...
  rug: {
    url: "/assets/models/glb/MTB-TBASE.glb",
    rotationOffsetY: 0,
    scale: 1,
  },
  bed: {
    url: "/assets/models/glb/special/bedDouble.glb",
    rotationOffsetY: 0,
    scale: 1,
  },
  sofa: {
    url: "/assets/models/glb/special/loungeSofa.glb",
    rotationOffsetY: 0,
    scale: 1,
  },
  media: {
    url: "/assets/models/glb/special/cabinetTelevision.glb",
    rotationOffsetY: 0,
    scale: 1,
  },
  lamp: {
    url: "/assets/models/glb/special/lampRoundFloor.glb",
    rotationOffsetY: 0,
    scale: 1,
  },
};
...
export function PlacedItem({ item }: { item: PlacedItemType }) {
  ...
  const registerObject = usePlannerStore((s) => s.registerObject);
  const unregisterObject = usePlannerStore((s) => s.unregisterObject);
  ...
  const fallbackDims: [number, number, number] = useMemo(() => {
  ...
  // Register this object's ref in the store for external controls (TransformControls)
  useEffect(() => {
    if (!groupRef.current) return;
    registerObject(item.id, groupRef.current);
    return () => unregisterObject(item.id);
  ...
  useEffect(() => {
    ...
    if (minY < 0) {
      const dy = -minY;
      setItemTransform(item.id, {
        position: [item.position[0], dy, item.position[2]],
      });
      ...
      onPointerMove={(e) => {
        ...
        if (item.kind === "door" || item.kind === "window") {
          ...
          if (snap && snap.dist < 0.6) {
            ...
            const y =
              item.kind === "window" ? Math.max(1.1, item.position[1]) : 0;
            setItemTransform(item.id, {
              position: [newX, y, newZ],
              rotation: [0, normalAngle, 0],
            });
            return;
          }
        ...
        setItemTransform(item.id, {
          position: [newX, Math.max(0, item.position[1]), newZ],
        });
      }}
    >
      {/* Selection ring */}
