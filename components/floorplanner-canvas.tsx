  ...
  return (
    <div className="relative w-full h-[100svh] md:h-full select-none">
        ...
        onPointerMove={(e) => {
          ...
          if (dragMode === "CORNER" && dragIndex !== null) {
            ...
            const normalProjection =
              dragDelta.x * wallNormal.x + dragDelta.z * wallNormal.z;
            ...
            // Move both endpoints of the wall using indices from segment order
            // segs are built from consecutive pts (0..n-2) mapping to (i,i+1)
            const aIndex = dragWallIndex;
            const bIndex = (dragWallIndex + 1) % pts.length;

            if (aIndex >= 0) {
              movePoint(aIndex, [
                dragStartPoints[aIndex].x + constrainedDelta.x,
                dragStartPoints[aIndex].z + constrainedDelta.z,
              ]);
            }
            if (bIndex >= 0 && bIndex < dragStartPoints.length) {
              movePoint(bIndex, [
                dragStartPoints[bIndex].x + constrainedDelta.x,
                dragStartPoints[bIndex].z + constrainedDelta.z,
              ]);
            }
          }
      ...
      <div className="absolute bottom-4 right-4 bg-white/80 backdrop-blur-sm p-2 rounded-md shadow-md text-sm">
        ...
        {isShiftPressed && (
          <div className="text-blue-600 text-xs mt-1">Grid Snap: ON</div>
        )}
        <div className="text-xs text-gray-500 mt-1">
          ESC: Cancel • Shift: Grid Snap
        </div>
      </div>
    </div>
  );
